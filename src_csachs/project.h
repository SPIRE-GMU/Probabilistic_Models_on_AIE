...
public:
  ...
  simpleGraph()
  {
    ...
    cliq_3 = kernel::create(cliq2);
    cliq_4 = kernel::create(cliq2);
    cliq_5 = kernel::create(cliq2);
    cliq_6 = kernel::create(cliq3);
    ...
    connect<stream> net0 (in.out[0], cliq_1.in[0]);
    connect<stream> net1 (cliq_1.out[1],cliq_2.in[0]);
    connect<stream> net2 (cliq_2.out[1],cliq_3.in[0]);
    connect<stream> net3 (cliq_3.out[1],cliq_4.in[0]);
    connect<stream> net4 (cliq_4.out[1],cliq_5.in[0]);
    connect<stream> net5 (cliq_5.out[1],cliq_6.in[0]);

    connect<stream> net6 (cliq_6.out[0], cliq_5.in[1]); //
    connect<stream> net7 (cliq_5.out[0], cliq_4.in[1]); //
    connect<stream> net8 (cliq_4.out[0], cliq_3.in[1]); //
    connect<stream> net9 (cliq_3.out[0], cliq_2.in[1]); //

    connect<stream> net10 (cliq_2.out[0], cliq_1.in[1]); //

    connect<stream> net11 (cliq_1.out[0], out.in[0]); //
    connect<stream> net12 (cliq_1.out[0], out0.in[0]); //
    ...
    source(cliq_6) = "src/kernels/cliq3.cc";
    source(cliq_1) = "src/kernels/cliq1.cc";
    source(cliq_2) = "src/kernels/cliq2.cc";
    source(cliq_3) = "src/kernels/cliq2.cc";
    source(cliq_4) = "src/kernels/cliq2.cc";
    source(cliq_5) = "src/kernels/cliq2.cc";

    // runtime<ratio>(repeat_node) = 0.9;
    runtime<ratio>(cliq_6) = 0.9;
